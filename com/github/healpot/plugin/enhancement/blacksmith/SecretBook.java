...
public class SecretBook {
	...
	public void addFailstackToStorage(Main m, Player player) {
		m.sendMessage(SettingsManager.lang.getString("Config.pluginTag")
				+ SettingsManager.lang.getString("Save.createFailstack").replaceAll("%failstack%",
	...
	public void loadStorage(Main m, Player player) {
		...
		if (SettingsManager.data.contains("AdviceOfValks." + player.getName()) || storage.containsKey(player)) {
			Scanner sc = new Scanner(SettingsManager.data.getString("AdviceOfValks." + player.getName()));
			while (sc.hasNext()) {
	...
	public void saveStorageToDisk(Main m, Player player, boolean save) {
		...
		if (working.size() > 0) {
			...
			SettingsManager.data.set("AdviceOfValks." + player.getName(), result);
		}
	...
	public void list(Main m, Player player, int pageNumber) {
		...
		m.sendMessage(SettingsManager.lang.getString("Config.pluginTag")
				+ SettingsManager.lang.getString("Save.failstackTitle").replaceAll("%page%", Integer.toString(page)),
				player);
		for (Integer fs : adviceOfValks) {
			count++;
			m.sendMessage(SettingsManager.lang.getString("Config.pluginTag")
					+ SettingsManager.lang.getString("Save.listing").replaceAll("%NUMBER%", Integer.toString(count))
	...
	public void select(Main m, Player player, int selectedFailstack) {
		if (selectedFailstack > 0 && m.failstack.getLevel(m, player) == 0) {
			try {
				...
				m.sendMessage(SettingsManager.lang.getString("Config.pluginTag")
						+ SettingsManager.lang.getString("Valks.noAdvice"), player);
			}
		} else
			m.sendMessage(SettingsManager.lang.getString("Valks.noAdvicce")
