...
public class Menu {
	...
	public void showEnhancingMenu(Main m, Player player, ItemStack item) {
		screen = Bukkit.getServer().createInventory(null, 27, SettingsManager.lang.getString("Menu.gui.title"));
		createMenu(m, player);
	...
	public void createMenu(Main m, Player player) {
		screen.clear();
		enhance = createItem(DyeColor.YELLOW, ChatColor.YELLOW + SettingsManager.lang.getString("Menu.gui.enhance"));
		ItemMeta enhanceim = enhance.getItemMeta();
		List<String> enhanceStr = new ArrayList<String>();
		enhanceStr.add(m.toColor(SettingsManager.lang.getString("Menu.lore.ifSuccess")));
		enhanceStr.add(m.toColor(SettingsManager.lang.getString("Menu.lore.ifFail")));
		enhanceStr.add(m.toColor(SettingsManager.lang.getString("Menu.lore.ifDowngrade")));
		enhanceStr.add(m.toColor(SettingsManager.lang.getString("Menu.lore.ifDestroy")));
		enhanceim.setLore(enhanceStr);
		enhance.setItemMeta(enhanceim);

		force = createItem(DyeColor.PURPLE, ChatColor.RED + SettingsManager.lang.getString("Menu.gui.force"));
		ItemMeta forceim = force.getItemMeta();
		List<String> forceStr = new ArrayList<String>();
		forceStr.add(m.toColor(SettingsManager.lang.getString("Menu.lore.force1")));
		forceim.setLore(forceStr);
		force.setItemMeta(forceim);

		stats = createItem(DyeColor.LIGHT_BLUE, ChatColor.AQUA + SettingsManager.lang.getString("Menu.gui.stats"));
		...
		statsStr.add(m.toColor(SettingsManager.lang.getString("Menu.lore.stats1")));
		statsStr.add(m.toColor(SettingsManager.lang.getString("Menu.lore.stats2")));
		statsim.setLore(statsStr);
		stats.setItemMeta(statsim);

		remove = createItem(DyeColor.RED, ChatColor.RED + SettingsManager.lang.getString("Menu.gui.remove"));
		ItemMeta removeim = remove.getItemMeta();
		List<String> removeStr = new ArrayList<String>();
		removeStr.add(m.toColor(SettingsManager.lang.getString("Menu.lore.remove")));
		...
		storeim.setDisplayName(SettingsManager.lang.getString("Menu.gui.store"));
		List<String> storeStr = new ArrayList<String>();
		storeStr.add(m.toColor(SettingsManager.lang.getString("Menu.lore.store1")));
		storeStr.add(m.toColor(SettingsManager.lang.getString("Menu.lore.store2")));
	...
	public void updateForce(Main m, ItemStack item, Player player) {
		if (m.permissions.commandForce(m, player)) {
			...
			if (enchantLevel < 7 || enchantLevel > 16) {
				...
				update.add(m.toColor(SettingsManager.lang.getString("Menu.lore.force1")));
				update.add(m.toColor(SettingsManager.lang.getString("Menu.lore.force2")
	...
	public void updateEnhance(Main m, ItemStack item, Player player) {
		...
		List<String> update = new ArrayList<String>();
		update.add(m.toColor(SettingsManager.lang.getString("Menu.lore.ifSuccess")));
		update.add(m.toColor(SettingsManager.lang.getString("Menu.lore.ifFail")));
		if (m.enhance.getItemEnchantLevel(m, player, item) > 15) {
			update.add(m.toColor(SettingsManager.lang.getString("Menu.lore.ifDowngrade")));
		}
		im.setLore(update);
	...
	public ItemStack stoneVisualized(Main m, int stoneId, Player player) {
		...
		im.setDisplayName(m.toColor(SettingsManager.lang.getString("Item." + stoneId)));
