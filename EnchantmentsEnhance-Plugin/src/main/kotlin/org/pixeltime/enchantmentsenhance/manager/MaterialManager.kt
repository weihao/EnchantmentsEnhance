...
class MaterialManager {
    ...
    companion object {
        ...
        @JvmStatic
        fun setUp() {
            for (s in SettingsManager.config.getStringList("material.stoneTypes")) {
                stoneTypes.add(XMaterial.searchByType(s)!!.toBukkit())
            }
            for (s in SettingsManager.config.getStringList("material.armor")) {
                armor.add(XMaterial.searchByType(s)!!.toBukkit())
            }
            for (s in SettingsManager.config.getStringList("material.weapon")) {
                weapon.add(XMaterial.searchByType(s)!!.toBukkit())
            }
            for (s in SettingsManager.config.getStringList("material.bow")) {
                bow.add(XMaterial.searchByType(s)!!.toBukkit())
            }
            for (s in SettingsManager.config.getStringList("material.hoe")) {
                hoe.add(XMaterial.searchByType(s)!!.toBukkit())
            }
            for (s in SettingsManager.config.getStringList("material.pickaxe")) {
                pickaxe.add(XMaterial.searchByType(s)!!.toBukkit())
            }
            for (s in SettingsManager.config.getStringList("material.axe")) {
