...
enum class XMaterial(val alias: String, val data: Int) {
    ...
    fun parseItem(): ItemStack {
        val mat = parseMaterial()
        return if (isNewVersion()) {
            ItemStack(mat)
        } else ItemStack(mat, 1, data.toByte().toShort())
    }

    fun isSameMaterial(comp: ItemStack): Boolean {
        if (isNewVersion()) {
            return comp.type == this.parseMaterial()
        }
        if (comp.type == this.parseMaterial() && comp.data.data.toInt() == this.data) {
            return true
        }
        val xmat = fromMaterial(comp.type)
        if (isDamageable(xmat!!)) {
            if (this.parseMaterial() == comp.type) {
                return true
            }
        }
        return false
    ...
    fun fromMaterial(mat: Material): XMaterial? {
        try {
            return XMaterial.valueOf(mat.toString())
        } catch (e: IllegalArgumentException) {
            for (xmat in XMaterial.values()) {
                if (xmat.alias == mat.toString()) {
                    return xmat
                }
            }
        }
    ...
    fun isDamageable(type: XMaterial): Boolean {
        when (type.toString()) {
            "HELMET" -> return true
            "CHESTPLATE" -> return true
            "LEGGINGS" -> return true
            "BOOTS" -> return true
            "SWORD" -> return true
            "AXE" -> return true
            "PICKAXE" -> return true
            ...
            "HOE" -> return true
            "ELYTRA" -> return true
            "TURTLE_HELMET" -> return true
            "TRIDENT" -> return true
            "HORSE_ARMOR" -> return true
            "SHEARS" -> return true
            else -> return false
        }
    }
    ...
    fun parseMaterial(): Material {
        val mat = Material.matchMaterial(this.toString())
        return mat ?: Material.matchMaterial(alias)
    }

    companion object {

        internal var newV = -1
        private val cachedSearch = HashMap<String, XMaterial>()

        fun isNewVersion(): Boolean {
            if (newV == 0) return false
            if (newV == 1) return true
            val mat = Material.matchMaterial("RED_WOOL")
            if (mat != null) {
                newV = 1
                return true
            }
        ...
        fun requestXMaterial(name: String, data: Byte): XMaterial? {
            if (cachedSearch.containsKey(name.toUpperCase() + "," + data)) {
                return cachedSearch[name.toUpperCase() + "," + data]
            }
            for (mat in XMaterial.values()) {
                if (name.toUpperCase() == mat.alias && mat.data.toByte() == data) {
                    cachedSearch[mat.alias + "," + data] = mat
                    return mat
                }
            }
            return null
        ...
        fun fromString(key: String): XMaterial? {
            return try {
                XMaterial.valueOf(key)
            } catch (e: IllegalArgumentException) {
                val split = key.split(":".toRegex()).dropLastWhile { it.isEmpty() }.toTypedArray()
                if (split.size == 1) {
                    requestXMaterial(key, 0.toByte())
                } else {
                    requestXMaterial(split[0], Integer.parseInt(split[1]).toByte())
                }
            }
